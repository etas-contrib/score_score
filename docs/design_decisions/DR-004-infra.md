...
* Tooling repo
  ...
  * Which is used by a bazel rule named `copyright_checker`
...
* Module repo
  ...
  * Has a `copyright.yml` which runs on pull_request. It calls `cicd-workflows/copyright.yml` and passes the exact name of the local `copyright` target.
* `cicd-workflows` repo
...
Is this too complicated? Let's disect the requirements:
